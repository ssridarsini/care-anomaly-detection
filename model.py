...
class Model(nn.Module):
    def __init__(self, n_in, n_h, activation, readout, config):
        ...
        self.diffpool = DiffPoolingBlock(
            n_in, n_clusters=config['clusters'], dim=-1)
        self.ReLU = nn.ReLU()
        if readout == 'max':
    ...
    def forward(self, seq, adj, raw_adj, adj2=None,  raw_adj2=None, sparse=False):
        ...
        cluster_sim = self.alpha * \
            torch.mm(assign, assign.T) + raw_adj * (1 - self.alpha)
        if self.norm:
    ...
    def evaluation(self, score, ano_label):
        ...
        AP = average_precision_score(
            ano_label, score, average='macro', pos_label=1, sample_weight=None)
        print('AP::{:.4f}, AUC:{:.4f}'.format(AP, auc))
        

    # ====================== model.py — patched piece ======================

    def graph_contrastive_loss(self, v1, v2, assignment, adj, test_phase=False):
        if v1.shape[0] > 10000 and not test_phase:
            idx = torch.randperm(v1.shape[0], device=v1.device)[
                                :8000]   # changed (from eha)
        else:
            # changed (from eha)
            idx = torch.arange(v1.shape[0], device=v1.device)
        v1, v2, assignment = v1[idx], v2[idx], assignment[idx]
        adj = adj[idx, :][:, idx]
        v = (v1 + v2)/2
        sim = self.cosine_sim(v, v, self.tau)
        O = self.alpha * torch.matmul(assignment,
                                    assignment.T) + (1 - self.alpha) * adj
...
'''
    def graph_contrastive_loss(self, v1, v2, assignment, adj, test_phase=False):
        ...
        loss = torch.norm(O / O.sum(dim=1).clamp_min(0.01) - \
                          sim, 2, dim=1).mean()
        return loss
'''
