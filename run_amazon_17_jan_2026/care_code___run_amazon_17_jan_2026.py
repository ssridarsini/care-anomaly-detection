...
# Global configuration (default values, can be overridden by CLI args)
CUT = 25      # Number of cuts (ensembles) for the model
EPOCH = 500   # Number of training epochs per cut
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open(f"log/{script_name}_cut_{CUT}_epoch_{EPOCH}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log", "a")
...
if EPOCH < 50:
    print("!"*80)
    print(f"WARNING: EPOCH is set to {EPOCH}. The model will likely NOT converge.")
    print("Recommended EPOCH > 50 for meaningful results. Expect poor AUC/AP.")
    print("!"*80)
...
def load_dataset(args):
    ...
    if args.dataset == 'Amazon':
        ...
        #config['cutting'] = 25
        config['cutting'] = min(getattr(args, 'cutting', CUT), CUT)  # updated here
        config['lamb'] = 1
        config['alpha'] = 0.8
        config['norm'] = False
...
def graph_nsgt(dis_array, adj):
    ...
    # Optimization: Use masked selection to avoid allocating dense NxN tensor for dis_array * adj
    # This reduces peak memory usage significantly (e.g. 784MB -> 196MB for mask + small 1D tensor)
    mask = adj > 0
    nonzero = dis_array[mask]
    
    mean_dis = nonzero.mean() if nonzero.numel() > 0 else torch.tensor(0.0, device=adj.device)
...
def main(args):
    ...
    if raw_adj1 is not None:
        raw_adj1 = raw_adj1.to(device)
    if raw_adj2 is not None:
        raw_adj2 = raw_adj2.to(device)
    ...
    if os.path.exists(distance1_file):
        dis_array1 = torch.tensor(np.load(distance1_file), dtype=torch.float32).to(device)
    else:
    ...
    if raw_adj2 is not None:
        ...
        if os.path.exists(dist2_path):
            dis_array2 = torch.tensor(np.load(dist2_path), dtype=torch.float32).to(device)
        else:
    ...
    for n_cut in range(config['cutting']):
        ...
        # Explicit cleanup after heavy graph operation
        gc.collect()
        torch.mps.empty_cache()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cutting', type=int, default=CUT)         # kept for CLI parity; config['cutting'] is used internally
