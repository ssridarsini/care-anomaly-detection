...
# ====================== utils.py — patched pieces ======================

def calc_distance(adj, seq):
    # create on same device/dtype as inputs                                    # changed (from eha)
    dis_array = torch.zeros((adj.shape[0], adj.shape[1]), device=adj.device, dtype=seq.dtype)  # changed (from eha)
    row = adj.shape[0]
    for i in range(row):
        node_index = torch.argwhere(adj[i, :] > 0)
        for j in node_index:
            diff = seq[i] - seq[j]
            dis = torch.sqrt(torch.sum(diff * diff))
            dis_array[i][j] = dis
    return dis_array


def graph_nsgt(dis_array, adj):
    # put dis_array on the same device as adj                                   # changed (from eha)
    dis_array = dis_array.to(adj.device)                                        # changed (from eha)
    row = dis_array.shape[0]
    dis_array_u = dis_array * adj
    mean_dis = dis_array_u[dis_array_u != 0].mean()
    for i in range(row):
        ...
        if node_index.shape[0] != 0:
            ...
            min_dis = mean_dis
            if max_dis > min_dis:
                random_value = (max_dis - min_dis) * np.random.random_sample() + min_dis.item()
                cutting_edge = torch.argwhere(dis_array[i, node_index[:, 0]] > random_value)
                if cutting_edge.shape[0] != 0:
                    adj[i, node_index[cutting_edge[:, 0]]] = 0
    adj = adj + adj.T
